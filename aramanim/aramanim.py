...
class Segment(Line):
    def __init__(self, start=0.5*LEFT, end=0.5*RIGHT, label=None, label_font_size=DEFAULT_LABEL_FONT_SIZE, **kwargs):
        ...
        super().__init__(start, end, buff=0, **kwargs)

        self.start_edge = Line(ORIGIN, edge_hight*stroke_width/5*normal_direction, **kwargs).move_to(start)
        self.end_edge = Line(ORIGIN, edge_hight*stroke_width/5*normal_direction, **kwargs).move_to(end)
        self.add(self.start_edge, self.end_edge)
...
class Scissors(VGroup):
    def __init__(self, cut_point, **kwargs):
        self.scissor_1 = SVGMobject('siz.svg').set_color(WHITE)
        self.scissor_2 = SVGMobject('siz2.svg').set_color(WHITE)
