    ...
    def __init__(
        ...
        class Drawing(VGroup):
            def __init__(self):
                ...
                self.first_segment = Segment(
                        ...
                        np.array(first_segment_start_point)
                        + np.array([equal_segment_length, 0, 0])
                    )

                self.second_segment = self.first_segment.copy(
                    ).shift(1.5 * DOWN)

                self.extra_segment = Segment(
                    self.second_segment.endmark_right.get_center(),
                    self.second_segment.endmark_right.get_center()
                    + np.array([extra_segment_length, 0, 0]),
                    color=extra_segment_color,
                    text=MathTex(f'{extra_length}',
                                 font_size=segment_length_font_size)
                )

                self.dashed_lines = VGroup(
                        DashedLine(self.first_segment.get_left(),
                                   self.second_segment.get_left()),
                        DashedLine(self.first_segment.get_right(),
                                   self.second_segment.get_right())
                    )
                ...
                self.brace = BraceLabel(
                    VGroup(self.first_segment, self.extra_segment),
                    brace_direction=RIGHT,
                    buff=1,
                    text=f'{total_length}',
                    font_size=total_length_font_size
                )
        ...
        class Solution(VGroup):
            def __init__(
                ...
                self.total_minus_extra.next_to(drawing.second_segment,
                                               5 * DOWN,
                                               aligned_edge=LEFT)
                ...
                self.divide_by_two.next_to(self.total_minus_extra,
                                           1.5 * DOWN,
                                           aligned_edge=LEFT)
                ...
                self.equal_length_plus_extra.next_to(self.divide_by_two,
                                                     1.5 * DOWN,
                                                     aligned_edge=LEFT)
                ...
                self.count_one_part[1].next_to(
                    self.count_one_part[0],
                    RIGHT,
                    buff=0.25
                )

                self.first_segment_text = MathTex(
                    f'{equal_length}',
                    font_size=segment_length_font_size
                )
                self.first_segment_text.next_to(
                    drawing.first_segment,
                    DEFAULT_SEGMENT_TEXT_POSITION
                )

                self.second_segment_text = MathTex(
                    f'{equal_length}',
                    font_size=segment_length_font_size
                )
                self.second_segment_text.next_to(
                    drawing.second_segment,
                    DEFAULT_SEGMENT_TEXT_POSITION
                )

                self.first_rect = SurroundingRectangle(
                    self.first_segment_text,
                    GREEN,
                    SMALL_BUFF + 0.05,
                    0.1
                )
                self.second_rect = SurroundingRectangle(
                    drawing.second_segment_combined.text,
                    GREEN,
                    SMALL_BUFF + 0.05,
                    0.1
                )
                ...
                self.scissors = Scissors(
                    drawing.second_segment.endmark_right.get_center()
                )
