...
class Multiplication(Scene):
    ...
    def construct(self):
        ...
        task = MathTex("7 \\cdot 134 = ", "938", font_size=115)
        ...
        multiplication_tex.arrange(DOWN, aligned_edge=LEFT, buff=.4).to_edge(DL).shift(UP*.5)
        ...
        first_number = multiplication_tex[2][8:10].copy()
        second_number = multiplication_tex[2][4:7].copy()
        third_number = multiplication_tex[2][0:3].copy()

        for index, tex in enumerate(multiplication_tex):
            if index == 3:
                self.play(Write(tex[:4]))
                self.wait()
                self.play(Indicate(multiplication_tex[2][4:7]))
                self.wait()
                self.play(Write(tex[4:11]))
                self.wait()
                self.play(Indicate(multiplication_tex[2][8:10]))
                self.wait()
        ...
        numbers_in_red = VGroup(multiplication_tex[3][11:13], multiplication_tex[4][14:16])
        numbers_in_orange = VGroup(multiplication_tex[3][4:7], multiplication_tex[4][5:8])
        ...
        self.play(
            multiplication_tex[-1][-3:].animate.set_color(YELLOW),
            answer.animate.set_color(YELLOW)
        )
        self.play(
            Indicate(multiplication_tex[-1][-3:]),
            Indicate(answer)
        )
        self.wait()

        self.play(answer.copy().animate.scale(115/FONT_SIZE).move_to(task[-1].get_center()))
