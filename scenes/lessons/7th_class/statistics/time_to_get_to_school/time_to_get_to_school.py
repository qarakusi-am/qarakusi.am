...
class TimeToGetToSchool(Scene):
    def construct(self):
        ...
        width_equality=str(minutes[len(minutes)-1])+ " - "+str(minutes[0])+ " = "+str(minutes[len(minutes)-1]-minutes[0])
        ...
        self.play(FadeIn(sum_fraction_line.next_to(sum_nominator,DOWN).scale(12)),Write(sum_denominator.next_to(sum_fraction_line,DOWN)))
        self.wait()
    ...
    def get_elements_from_list(self,minutes):
        ...
        return {"first_element":first_element,"last_element":last_element,"mode_value":mode_value,"mode_numbers":mode_numbers,"total_minutes_text":total_minutes_text}
