from manim import Scene, FadeIn, Tex, Write, VGroup, AnimationGroup, \
    ReplacementTransform, ORANGE, FadeOut, CurvedArrow, SurroundingRectangle, YELLOW
...
class Problem12804(Scene):
    def construct(self):
        ...
        bookshelf = SimpleSVGMobject("star").shift(DOWN)
        ...
        self.play(Write(problem_settings[0], run_time=2.2))
        self.wait(1.5)

        self.play(Write(problem_settings[1], run_time=2))
        self.wait(1.5)

        self.play(Write(problem_settings[2], run_time=1))
        self.wait(2)
        ...
        helpers = VGroup(
            ...
            Tex("$5x - 7x = - 56 + 20$", font_size=FORMULA_FONT_SIZE).move_to(converted_setting_part1).shift(
                DOWN * 1.4 + RIGHT * 2),
            Tex("$-2x = -36$", font_size=FORMULA_FONT_SIZE).move_to(converted_setting_part1).shift(DOWN * 2.2 + RIGHT * 1.75),
        ...
        for i in range(0, 2):
            self.play(Write(helpers[i], run_time=1.5))
            self.wait()
        self.wait()
        ...
        self.play(Write(helpers[2], run_time=1.5))
        self.wait(2)
        ...
        solution = Tex(solution_str, font_size=FORMULA_FONT_SIZE).shift(UP * 2.5)
    ...
    def convert_problem_setting_1(self):
        ...
        self.converted_setting_part1 = converted_setting_part1 = VGroup(
            ...
            Tex("$\cdot 5 $", font_size=FORMULA_FONT_SIZE).next_to(problem_settings[0], RIGHT).shift(LEFT * 0.2)
        )

        surrounding_rectangles = VGroup(SurroundingRectangle(problem_settings[0], color=YELLOW, buff=0.15),
                                        SurroundingRectangle(problem_settings[1], color=YELLOW, buff=0.15))

        self.play(Write(surrounding_rectangles[0]))
        self.wait(2)
        self.play(AnimationGroup(FadeOut(surrounding_rectangles[0]),
            ReplacementTransform(problem_settings[0], converted_setting_part1[0][0][1:-1]),
            ))
        self.wait()
        ...
        self.play(ReplacementTransform(problem_settings[1], converted_setting_part1[1]),
        ...
        self.play(AnimationGroup(self.book_count_variable[0].animate.shift(UP*2.5).scale(1.4)),
                  converted_setting_part1.animate.next_to(self.equality_sign[0],LEFT).shift(DOWN))
        self.wait(2)
    ...
    def convert_problem_setting_2(self):
        ...
        self.converted_setting_part2 = converted_setting_part2 = VGroup(
            Tex("$(x - 8)$", font_size=FORMULA_FONT_SIZE).next_to(problem_settings[1], LEFT).shift(RIGHT * 0.1),
            Tex("$\cdot 7 $", font_size=FORMULA_FONT_SIZE).next_to(problem_settings[0], RIGHT).shift(LEFT * 0.13)
        )
        ...
        self.wait(2)
        self.play(AnimationGroup(
            ReplacementTransform(problem_settings[0], converted_setting_part2[0][0][1:-1]),
        ...
        self.play(ReplacementTransform(problem_settings[1], converted_setting_part2[1]),
                  FadeOut(surrounding_rectangles[1]),
                  AnimationGroup(
