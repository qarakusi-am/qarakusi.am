...
class Problem12754(Scene):
    def construct(self):
        ...
        graph1 = coordinate_axes.plot(lambda t: 2 * t + 4,
                                      x_range=[-3, 5],
                                      color=ORANGE)
        formula1 = MathTex("y=2x+4", font_size=FONT_SIZE, color=ORANGE).move_to(graph1).shift(RIGHT * 0.5 + UP * 2.3)
        ...
        line_intersection_point1 = VGroup(
            ...
            MathTex("(x,", "2x + 4", ")").scale(0.7).next_to(coordinate_axes.c2p(1, 6, 0)))
        ...
        self.play(Write(line_intersection_point1,run_time=RUN_TIME_SPEED))
        self.wait()

        self.play(Write(line_intersection_point2,run_time=RUN_TIME_SPEED))
        self.wait()

        self.play(AnimationGroup(VGroup(line_intersection_point1[0], line_intersection_point2[0]).animate(run_time=2.5).shift(RIGHT * 2.38),
                                 # VGroup(line_intersection_point1[1],line_intersection_point2[1]).animate(run_time=2.5).move_to(graphs_intersection_point),
                                 VGroup(line_intersection_point1[2],line_intersection_point2[2]).animate(run_time=2.5).move_to(graphs_intersection_point).shift(RIGHT)))
        self.wait(2)
        ...
        equations = VGroup(MathTex("2x+4=4.5x-7", font_size=FONT_SIZE).shift(UP * 3 + LEFT * 4),
                           MathTex("2x-4.5x=-7-4", font_size=FONT_SIZE).shift(UP * 2.2 + LEFT * 4.5),
                           MathTex("-2.5x=-11", font_size=FONT_SIZE).shift(UP*1.4 + LEFT *4.43),
                           MathTex(r"x=\frac{-11}{-2.5}=4.4", font_size=FONT_SIZE).shift(UP*0.4+LEFT * 5),
                           MathTex("2x+4=2\cdot 4.4+4=12.8", font_size=FONT_SIZE).shift(DOWN * 0.6 + LEFT*3.05 )
                           )
        ...
        self.play(VGroup(coordinate_axes,graph1,graph2,formula1,formula2,graphs_intersection_point,point_x1,
                         line_intersection_point1,line_intersection_point2).animate.shift(RIGHT*2))
        self.wait(2)
        ...
        self.play(Write(graphs_intersection_point_label))
        self.wait(2)
